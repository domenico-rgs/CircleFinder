...
public class View extends JFrame {
	...
	public View(ArrayList<BufferedImage> images, String dir) {
		...
		lab1.setVerticalAlignment(JLabel.TOP);
		...
		lab4.setVerticalAlignment(JLabel.TOP);
		...
		lab6.setVerticalAlignment(JLabel.TOP);
		resImg.add(lab6);
		...
		for (int counter = 0; counter < images.size(); counter++) {
			...
			if(img.getWidth() > img.getHeight()) {
				if(img.getWidth() > maxW) {
					Image tmp = img.getScaledInstance(maxW, (int)(img.getWidth()/img.getHeight()*maxW), Image.SCALE_SMOOTH);
					BufferedImage dimg = new BufferedImage(maxW, (int)(img.getWidth()/img.getHeight()*maxW), BufferedImage.TYPE_INT_ARGB);
				...
				if(img.getHeight() > maxH) {
					Image tmp = img.getScaledInstance((int)(img.getHeight()/img.getWidth()*maxH), maxH, Image.SCALE_SMOOTH);
					BufferedImage dimg = new BufferedImage((int)(img.getHeight()/img.getWidth()*maxH), maxH, BufferedImage.TYPE_INT_ARGB);
