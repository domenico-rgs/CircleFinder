...
public class GaussianBlur {
	...
	public static BufferedImage blur(BufferedImage image) {
		if (kernel.length % kernelWidth != 0) {
			throw new IllegalArgumentException("Kernel dimensions are not valid");
		}
		...
		for (int h = height - kernel.length / kernelWidth + 1, w = width - kernelWidth + 1, y = 0; y < h; y++) {
			for (int x = 0; x < w; x++) {
				...
				for (int kernelIndex = 0, pixelIndex = y * width
						+ x; kernelIndex < kernel.length; pixelIndex += pxIdxOffset) {
